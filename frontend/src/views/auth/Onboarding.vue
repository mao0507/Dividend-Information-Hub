    ...
    <section class="p-6 md:p-10 flex items-center justify-center">
      <div class="w-full max-w-[560px] bg-surface-2 border border-border-strong rounded-[var(--radius)] p-6 md:p-8">
        <div class="flex items-center justify-between mb-6">
          <div>
            <div class="font-mono text-xs uppercase tracking-widest text-content-faint">Onboarding</div>
            <div class="text-lg text-content font-semibold mt-1">{{ stepTitles[step - 1] }}</div>
          </div>
          <button
            type="button"
        ...
        <div v-else-if="step === 2" class="space-y-4">
          ...
          <div class="flex gap-2">
            <Button text size="small" class="flex-1" @click="step = 1">上一步</Button>
            <Button size="small" class="flex-1" :loading="loading" @click="submitWatchlist">
              下一步
            </Button>
          </div>
        </div>
        ...
        <div v-else-if="step === 3" class="space-y-4">
          ...
          <div class="flex gap-2">
            <Button text size="small" class="flex-1" @click="step = 2">上一步</Button>
            <Button size="small" class="flex-1" :loading="loading" @click="submitPreference">
              下一步
            </Button>
          </div>
        </div>
        ...
        <div v-else class="space-y-4">
          ...
          <Button class="w-full" @click="goDashboard">
            進入 Dashboard
          </Button>
        </div>
      </div>
...
const stepTitles = ['帳號建立', '加入自選股', '設定提醒偏好', '完成']
const authModes = ['登入', '註冊'] as const
const authMode = ref<'登入' | '註冊'>('註冊')
