...
export class FinMindDividendSource implements DividendSource {
   ...
   */
  readonly sync = async (trackedCodes: Set<string>): Promise<DividendRecord[]> => {
    ...
    for (const code of trackedCodes) {
      ...
      for (const item of body.data ?? []) {
        ...
        const exDate = item.ex_dividend_trading_date
          ...
          : null
        const payDate = item.dividend_pay_date ? new Date(item.dividend_pay_date) : null
        const year = exDate ? exDate.getFullYear() : new Date(item.date).getFullYear()
