...
系統 MUST 以前端統一資料模型渲染 Dashboard 上方四個統計區塊，且每個區塊 MUST 至少包含 `title`、`displayValue`、`state`、`asOf` 四項欄位；不得在模板中直接消化 API 原始欄位。若涉及「累積股息收入」資料來源，系統 MUST 與持股 lot 管理口徑一致（以使用者持股與買入條件計算）。
...
系統 MUST 在資料缺漏、同步失敗或 freshness 驗證失敗時，對受影響區塊顯示中性值（例如 `--`）與狀態文案，不得顯示 `0` 作為缺值替代。對於「累積股息收入」區塊，若持股資料不足或明細尚未建立，系統 MUST 視為無可用收入資料並使用中性顯示。
