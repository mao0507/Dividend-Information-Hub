      ...
      <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div class="shrink-0 border-b border-border px-4 py-3 space-y-3">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-6 gap-2">
            <Select v-model="sectorStr" :options="SECTOR_OPTIONS" option-label="label" option-value="value" />
            <Select v-model="streakStr" :options="STREAK_OPTIONS" option-label="label" option-value="value" />
            <Select v-model="fillStr" :options="FILL_OPTIONS" option-label="label" option-value="value" />
            <Select v-model="capStr" :options="CAP_OPTIONS" option-label="label" option-value="value" />
          </div>
        </div>
        ...
        <div class="flex-1 overflow-auto p-4">
          ...
          >
            ...
            <Button size="small" @click="retryLoad">重新載入</Button>
          </div>
          <div
                  ...
                  >
                    ...
                    <td class="px-3 py-2">
                      ...
                      <div class="flex flex-wrap gap-1 mt-0.5">
                        <Chip :pt="{ root: { style: { color: '#94a3b8', background: 'rgba(148,163,184,0.12)' } } }">{{ row.sector }}</Chip>
                        <Chip v-if="row.badge" :pt="{ root: { style: { color: '#22c55e', background: 'rgba(34,197,94,0.12)' } } }">{{ row.badge }}</Chip>
                      </div>
                    </td>
                    ...
                    <td class="px-3 py-2" @click.stop>
                      <div class="flex gap-1">
                        <Button class="!px-2 !py-1 text-[10px]" @click="goStock(row.code)">詳</Button>
                      </div>
                    </td>
                  </tr>
                </tbody>
...
const normalizeRankingRow = (row: Partial<RankingStock>, idx: number): RankingStock => ({
  rank: Number.isFinite(row.rank) ? (row.rank as number) : idx + 1,
  code: row.code ?? '----',
  name: row.name ?? '未知名稱',
  sector: row.sector ?? '未知產業',
  freq: row.freq ?? 'annual',
  yield: Number.isFinite(row.yield) ? (row.yield as number) : 0,
  cash: Number.isFinite(row.cash) ? (row.cash as number) : 0,
  price: Number.isFinite(row.price) ? (row.price as number) : 0,
  changePct: Number.isFinite(row.changePct) ? (row.changePct as number) : 0,
  fillRate: Number.isFinite(row.fillRate) ? (row.fillRate as number) : 0,
  badge: row.badge,
