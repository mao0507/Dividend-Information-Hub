...
- [x] **FE-001** 建立 Vite + Vue 3 + TypeScript + Tailwind CSS 前端專案
...
- [x] **DB-001** 建立 PostgreSQL + Prisma Schema
...
- [x] **FE-003** 建立原子 UI 元件（`components/ui/`）
...
- [x] **FE-004** 建立佈局元件（`components/layout/`）
...
- [x] **FE-005** 建立圖表元件（`components/chart/`）
...
- [x] **FE-006** Dashboard 頁面（`pages/DashboardPage.vue`）
...
- [x] **FE-007** 除息行事曆頁面（`pages/CalendarPage.vue`）
...
- [x] **FE-008** 個股詳情頁面（`pages/StockDetailPage.vue`，高度 auto 可捲動）
  - 頂欄：麵包屑 + ★自選按鈕 + ⚡提醒按鈕
...
- [x] **FE-009** 高股息排行頁面（`pages/RankingPage.vue`）
...
- [x] **FE-010** 自選股管理頁面（`pages/WatchlistPage.vue`）
  - 摘要 4格（自選股總數/總市值/今年領息/待除息）
  - 側欄：分組清單（顏色點/名稱/數量）+ 新增分組
  - 分組卡片（可展開）：⋮⋮拖曳 / 名稱+代號+產業 / Spark / 股價+漲跌 / 下次除息 / 配息額 / 填息晶片 / ⚡⋮
  - 搜尋框（⌘K）+ 新增按鈕
  - 拖曳排序（SortableJS；`vue-draggable-plus` 於 Vite 編譯與 SFC 不相容故採用等效方案）
...
- [x] **FE-012** 視覺化分析頁面（`pages/VizPage.vue`）
  - 年份切換（2024/2025/2026）
...
- [x] **FE-013** 提醒中心頁面（`pages/AlertsPage.vue`）
...
- [x] **FE-014** Onboarding 頁面（`pages/OnboardingPage.vue`）
...
- [x] **FE-015** 設定頁面（`pages/SettingsPage.vue`）
...
- [x] **FE-016** Command Palette（`components/layout/CommandPalette.vue`）
  - 全域快捷鍵監聽（⌘K / Ctrl+K）
