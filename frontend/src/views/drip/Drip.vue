        ...
        <section class="bg-surface-2 border border-border rounded-[var(--radius)] p-5 space-y-4">
          ...
          <div class="space-y-1.5">
            <div class="flex justify-between items-center">
              <span class="text-[11px] text-content-soft">初始投入</span>
              <span class="font-mono text-xs text-accent">{{ fmtMoney(form.principal) }}</span>
            </div>
            <Slider v-model="form.principal" :min="0" :max="10000000" :step="10000" />
          </div>
          <div class="space-y-1.5">
            <div class="flex justify-between items-center">
              <span class="text-[11px] text-content-soft">每月加碼</span>
              <span class="font-mono text-xs text-accent">{{ fmtMoney(form.monthlyAdd) }}</span>
            </div>
            <Slider v-model="form.monthlyAdd" :min="0" :max="200000" :step="1000" />
          </div>
          <div class="space-y-1.5">
            <div class="flex justify-between items-center">
              <span class="text-[11px] text-content-soft">殖利率</span>
              <span class="font-mono text-xs text-accent">{{ fmtPct(form.yield) }}</span>
            </div>
            <Slider v-model="form.yield" :min="0" :max="12" :step="0.1" />
          </div>
          <div class="space-y-1.5">
            <div class="flex justify-between items-center">
              <span class="text-[11px] text-content-soft">成長率</span>
              <span class="font-mono text-xs text-accent">{{ fmtPct(form.growth) }}</span>
            </div>
            <Slider v-model="form.growth" :min="-5" :max="12" :step="0.1" />
          </div>
          <div class="space-y-1.5">
            <div class="flex justify-between items-center">
              <span class="text-[11px] text-content-soft">期間</span>
              <span class="font-mono text-xs text-accent">{{ fmtIntYear(form.years) }}</span>
            </div>
            <Slider v-model="form.years" :min="1" :max="30" :step="1" />
          </div>
          <div class="space-y-1.5">
            <div class="flex justify-between items-center">
              <span class="text-[11px] text-content-soft">稅率</span>
              <span class="font-mono text-xs text-accent">{{ fmtPct(form.taxRate) }}</span>
            </div>
            <Slider v-model="form.taxRate" :min="0" :max="40" :step="0.1" />
          </div>
        </section>
        ...
        <section class="space-y-4">
          <div class="bg-surface-2 border border-border rounded-[var(--radius)] p-5">
            <div class="flex flex-wrap gap-2 items-center">
              ...
              <Chip :pt="{ root: { style: { color: 'var(--accent)', background: 'rgba(34,197,94,0.12)' } } }">年領目標 {{ fmtCurrency(targetAnnualIncome) }}</Chip>
              <Chip :pt="{ root: { style: { color: '#3b82f6', background: 'rgba(59,130,246,0.14)' } } }">月均 {{ fmtCurrency(result?.averageMonthlyIncome ?? 0) }}</Chip>
              <Chip :pt="{ root: { style: {
                color: (result?.targetAchievementYear ?? 999) <= displayYears ? '#22c55e' : '#f59e0b',
                background: (result?.targetAchievementYear ?? 999) <= displayYears ? 'rgba(34,197,94,0.12)' : 'rgba(245,158,11,0.12)',
              } } }">
                {{ targetText }}
              </Chip>
              <div class="flex-1" />
...
import { dripApi } from '@/services/api/drip'
